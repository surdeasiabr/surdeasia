...
router.post('/', async (req, res) => {
    try {
        ...
        // Responde ao cliente IMEDIATAMENTE (o envio de email acontece em segundo plano)
        res.json({ success: true, message: 'Mensagem enviada com sucesso!' });

        // Envia os emails de forma assíncrona (não bloqueia a resposta)
        const emailToSurdeasia = sendEmail({
            to: process.env.EMAIL_USER,
            replyTo: email,
            subject: `Nova Consulta do Site - ${typeLabel}`,
            html: `
        ...
        let emailToClient = Promise.resolve();
        if (type === 'atacado') {
            emailToClient = sendEmail({
                to: email,
                subject: `Proposta Atacado - SURDEASIA`,
                html: `
                    <div style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; max-width: 600px; margin: 0 auto;">
        ...
        // Log result but don't block response
        Promise.all([emailToSurdeasia, emailToClient])
            .then(() => console.log(`✅ Emails enviados com sucesso para consulta de ${name} (${typeLabel})`))
            .catch(err => console.error(`❌ Erro ao enviar email em background:`, err.message));

    } catch (error) {
        console.error('Erro ao processar contato:', error);
        res.status(500).json({ success: false, error: error.message || 'Erro ao processar a solicitação.' });
    }
});
